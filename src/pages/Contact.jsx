...
export default function Contact() {
  ...
  useEffect(() => {
    document.title = `${t('contactPage.breadcrumb')} | Çobantaş`;
    return () => { document.title = 'Çobantaş | Gayrimenkul & İnşaat'; };
  }, [t]);

  const infoItems = [
    { icon: MapPin, label: t('contactPage.addressLabel'), value: contactInfo.address, href: MAP_LINK, external: true },
    { icon: Phone, label: t('contactPage.phoneLabel'), value: contactInfo.phoneDisplay, href: contactInfo.phoneHref },
    { icon: Mail, label: t('contactPage.emailLabel'), value: contactInfo.email, href: contactInfo.emailHref },
  ];
      ...
      <section className="border-b border-stone-200 bg-stone-100">
        <div className="mx-auto max-w-7xl px-6 py-16 md:py-20 lg:px-8">
          <nav className="mb-6 flex items-center gap-2 text-[0.7rem] font-semibold uppercase tracking-widestx text-ink-400">
            ...
            <span className="text-bronze-700">{t('contactPage.breadcrumb')}</span>
          </nav>
        ...
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="grid gap-14 lg:grid-cols-2 lg:gap-20">
            ...
            <div className="reveal">
              ...
              {/* Sosyal medya */}
              <div className="mt-12">
                <h4 className="mb-5 text-[0.7rem] font-semibold uppercase tracking-widestx text-ink-400">{t('contactPage.social')}</h4>
        ...
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="mb-8 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-end">
            <div>
              ...
              <h2 className="mt-6 font-serif text-3xl text-ink-900 md:text-4xl">{t('contactPage.visit')}</h2>
            </div>
            <a href={MAP_LINK} target="_blank" rel="noopener noreferrer"
              ...
              {t('contactPage.openMaps')} <ExternalLink className="h-4 w-4" />
            </a>
          </div>
